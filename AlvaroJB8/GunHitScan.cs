...
public class GunHitScan : NetworkBehaviour
{
    ...
    {
        timer = TickTimer.CreateFromSeconds(Runner, 0f);
    }
    void Update()
    {
        if (!Object.HasStateAuthority) return;
        
        bool canShoot = timer.Expired(Runner);

        if (Input.GetKey(KeyCode.Mouse0) && timer.Expired(Runner))
        {
            if (canShoot)
            {
                FireWeapon();
                timer = TickTimer.CreateFromSeconds(Runner, fireRate + 1.5f);
            }
        }
    }
